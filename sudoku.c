...
int sudoku_generate_board(Sudoku *self, int diffeculty){
    char* fileName = "sudoku.txt";
   	char* sudolin;
	   if((sudolin = sudoku_line(fileName)) == 0){
               printf("End Program\n");
               return 0;
           }
...
//	internal_sudoku_generate_finished_board(self);
    for (int i = 0; i < 81; i++){
    	self->board[i] = (int) sudolin[i] - 48;
    	self->finished_board[i] = (int) sudolin[i] - 48;
    }
...
char* sudoku_line(char *fileName){
                    FILE* source = NULL;
		    source = fopen(fileName,"r");
		    if(source == NULL)
		    {
		        printf("failed in opening the file or file not exists in system :%s",fileName);
                        fclose(source);
                        return 0;
		    }
		    ssize_t read;
		    ...
		    while((read = getline(&line, &len, source)) != -1)
		    {
		        ...
		        if(read == 83 || read == 81){
		        	...
		        	//printf("%s\n", numberline);
		        		for(l = 0; l < 82; l++){
		        			sudoDatenzeile[k][l] = numberline[l];
		        		}
		        		++k;
		        } else if(read < 30){
                            // KommentareZeile 
                        }else if(read < 81 || read >= 82){
                            ++m;
                            printf("SudoZeile %i nicht länger oder kürzer als 81. Bitte Dateifile korrigieren !", m);
                            fclose(source);
                            return 0;
		        }
		        //printf("%i\n", sizeof(numberline));
		        ...
		        for(i = 0; i < read-1; i++){
		           ...
		           if(!(numberline[i] >= '0' && numberline[i] <= '9')){
		        		++j;
		           }
		           if(read == 82 || read == 81){
		        	   if(((int) numberline[i] < 48) || ((int) numberline[i] > 57)){
		        	   	    printf("Sudoku Datenzeile corrupt\n");
                                            fclose(source);
                                            return 0;
		        	   }
		           }
		    ...
		    char* sudoline = (char*) malloc(sizeof(char) * 81);
		    ...
		    for(l = 0; l < 81; l++){
		    	sudoline[l] = sudoDatenzeile[random][l];
		    }
